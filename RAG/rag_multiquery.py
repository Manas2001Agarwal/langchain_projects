...
def take_unique_context(context_list):
    flattened_docs = [dumps(doc) for sublist in context_list for doc in sublist]
...
def context_retriver(index_name):
    ...
    Only provide these alternative questions (and no other text) in output separated by newlines. Original question: {question}
    """
    multi_query_prompt = PromptTemplate(
        template = multi_query_prompt,
        input_variables=["question"]
    )
    multiquery_retriever = multi_query_prompt | llm | StrOutputParser() | RunnableLambda(lambda x:x.split('\n')) | retriever.map() | RunnableLambda(take_unique_context)
